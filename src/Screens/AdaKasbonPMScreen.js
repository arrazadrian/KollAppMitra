...
const AdaKasbonPMScreen = ({ route }) => {

    const { 
        id_transaksi, hargalayanan, hargatotalsemua, namamitra, namatoko, namapelanggan, id_pelanggan, phonepelanggan
         } = route.params;
  ...
  const kasbonBaru =()=> {
    Alert.alert('Anda yakin buat kasbon baru?','Pastikan anda mengenal pelanggan tersebut dan dapat mempercayainya.',
          [
            ...
            {
              text: 'Yakin',
              onPress: PanggilMitraKasbon,
            }
          ]
          )
  }

  async function PanggilMitraKasbon(){
    try{
...
export default AdaKasbonPMScreen
