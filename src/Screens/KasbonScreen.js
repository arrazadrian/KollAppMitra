  ...
  useEffect(()=>{
    const fetchProses = async() => {
      try{
        ...
        const q = query(colRef, where("id_mitra", "==", auth.currentUser.uid), where("status_kasbon", "==", "Belum Lunas"), orderBy("waktu_dibuat","desc"));
        ...
        if (componentMounted.current){ // (5) is component still mounted?
          setKasbon(list); // (1) write data to state
      ...
      <FlatList
          ...
          ListEmptyComponent={ 
            <View style={{justifyContent:'center', alignItems:'center'}}>
              ...
              <Text style={styles.none}>Tidak ada kasbon yang belum lunas</Text> 
            </View>
          }
      />
      )}
